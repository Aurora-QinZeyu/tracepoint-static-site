...
const REVIEWED_FIELD_SEMANTICS={
  ...
  'payment.product':{meaning:'产生本次支付订单的产品或包体标识；矩阵包统一使用全小写值。',valueMeaningSource:'backend_contract',valueMode:'open',values:{popdoll:'PopDoll 包体；当前后端样例中的已知值'}},
  'payment.order_id':{meaning:'业务订单 ID；用于串联订单链路，也是“支付成功订单数”指标的去重键。',openValueSet:true},
  'payment.state':{meaning:'支付单在本次成功回传时的状态；payment 事件仅在支付成功后上报。',valueMeaningSource:'backend_contract',values:{paid:'支付已完成，订单进入成功态'}},
  'payment.channel_product_id':{meaning:'支付渠道侧的商品/SKU 标识。standard 表示标准订阅商品；cp_in_<积分数> 表示指定积分档位的站内加量包。该字段仍允许新增商品 ID。',valueMeaningSource:'backend_contract',valueMode:'open',values:{standard:'标准订阅商品',cp_in_30:'30 积分加量包',cp_in_100:'100 积分加量包'}},
  'payment.channel_price_id':{meaning:'支付渠道侧的价格 ID，用于区分订阅等级、周期、地区或价格实验；星号部分是动态价格方案后缀，不是字面上报值。',valueMeaningSource:'backend_contract',valueMode:'open',values:{'standard-subscribe-*':'Standard 订阅价格系列；* 为具体周期、地区或实验后缀','plus-subscribe-*':'Plus 订阅价格系列；* 为具体周期、地区或实验后缀'}},
  'payment.bill_reason':{meaning:'本次账单产生的业务原因，用于区分首购、自动续期、恢复订阅和周期末降级。',valueMeaningSource:'backend_contract',values:{create:'首次创建订单或新建订阅产生的账单',cycle:'订阅进入新计费周期时的自动续期账单',restart:'已取消或中断的订阅重新开始后产生的账单',cycle_at_end_down:'已预约降级，并在当前计费周期结束时执行降级产生的账单'}},
  'payment.country':{meaning:'支付用户所属国家/地区，采用大写 ISO 3166-1 alpha-2 两位代码；US、JP、CN 仅为示例，允许其他合法国家码。',valueMeaningSource:'backend_contract',valueMode:'open',values:{US:'美国',JP:'日本',CN:'中国'}},
